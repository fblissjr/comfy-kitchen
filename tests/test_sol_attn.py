...
def test_blk_cnt_hip_refuses():
    ...
    refusal beats a silent wrong number. This branch predates the HIP port,
    so the case skips unless a HIP backend with sol_attn is importable."""
    try:
        from comfy_kitchen.backends import hip as hip_backend
    except Exception:
        pytest.skip("no HIP backend on this branch")
    if not (hip_backend.is_available() and hasattr(hip_backend, "sol_attn")):
